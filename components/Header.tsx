        ...
        {isOpen && (
          <div className="md:hidden border-t border-slate-200 bg-white max-h-[80vh] overflow-y-auto">
            <nav className="flex flex-col p-6 gap-1" aria-label="Portfolio sections">
              <p className="text-[10px] font-bold uppercase tracking-widest text-slate-400 mb-2">This page</p>
              {NAV_LINKS.map((link) => (
                <a
                  key={link.label}
                  href={link.href}
              ...
              <p className="text-[10px] font-bold uppercase tracking-widest text-slate-400 mt-6 mb-1">
                Other sites (see top bar)
              </p>
              <p className="text-sm text-slate-500">
                Use the dark <strong className="text-slate-700">Portfolio</strong> strip at the top for Hub, Notebook,
                Configs, and writing.
              </p>
            </nav>
          </div>
